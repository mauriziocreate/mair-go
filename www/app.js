...
function bulkImportArtworksModal(){
  ...
  openModal('Importa più opere',`
    ...
    <div class="formgrid">
      ...
      const over=valid.filter(f=>f.size>IMG_MAX_BYTES);
      if(over.length){alert('Queste immagini superano 5 MB:\n'+over.map(f=>f.name).join('\n'));return;}
      ...
      const titles=[...document.querySelectorAll('[data-bulk-title]')].map(x=>x.value.trim());
      let imported=0,errors=[];
      for(let i=0;i<valid.length;i++){
        const f=valid[i];
        try{
          if(progress){progress.querySelector('span').style.width=Math.round((i/valid.length)*100)+'%';progress.querySelector('p').textContent=`Elaborazione ${i+1} di ${valid.length}: ${f.name}`;}
          const r=await processImage(f);
          const now=new Date().toISOString();
          db.artworks.unshift({
            id:uid(),title:titles[i]||titoloDaNomeFile(f.name),year:fd.get('bulkYear'),
      ...
      if(progress){progress.querySelector('span').style.width='100%';progress.querySelector('p').textContent=`Importate ${imported} opere su ${valid.length}.`;}
      await save();
  ...
  if(inp)inp.onchange=()=>{
    ...
    box.innerHTML=files.map((f,i)=>`<div class="bulk-artwork-row ${f.size>IMG_MAX_BYTES?'invalid':''}"><div><strong>${esc(f.name)}</strong><small>${(f.size/1048576).toFixed(2)} MB${f.size>IMG_MAX_BYTES?' · supera il limite':''}</small></div><input data-bulk-title value="${esc(titoloDaNomeFile(f.name))}" aria-label="Titolo opera ${i+1}"></div>`).join('');
  };
}
