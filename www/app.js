...
const I18N={
  ...
  'Organizza le tue mostre':'Organize your exhibitions',
  'Gestisci esposizioni, sedi, date, opere partecipanti e cataloghi.':'Manage exhibitions, venues, dates, participating artworks and catalogs.',
  'La tua biblioteca professionale':'Your professional library',
  'Gestisci PDF, documenti, immagini, cataloghi, testi e appunti.':'Manage PDFs, documents, images, catalogs, texts and notes.',
...
library:()=>`${section('Biblioteca Pro','<button class="btn primary" data-action="newLibrary">＋ Carica file</button>')}<section class="hero"><h2>La tua biblioteca professionale</h2><p>Gestisci PDF, documenti, immagini, cataloghi, testi e appunti.</p></section><div class="toolbar"><input id="libSearch" class="search" 'Cerca titolo, autore, tag, descrizione e note…')}"><select id="libType"><option value="">Tutti i file</option><option value="pdf">PDF</option><option value="doc">Documenti</option><option value="image">Immagini</option><option value="fav">Preferiti</option><option value="linked">Collegati alle opere</option></select><select id="libCat"><option value="">Tutte le categorie</option>${db.settings.lists.categories.map(x=>`<option>${esc(x)}</option>`)}</select></div><div id="libGrid" class="grid">${db.library.map(libCard).join('')||empty('📚','Carica PDF, DOCX, testi, immagini, cataloghi e schede tecniche.','<button class="btn primary" data-action="newLibrary">Carica il primo file</button>')}</div>`,
...
function exhibitionsView(){return `${section('Mostre','<button class="btn primary" data-action="newExhibition">＋ Nuova mostra</button>')}<section class="hero"><h2>Organizza le tue mostre</h2><p>Gestisci esposizioni, sedi, date, opere partecipanti e cataloghi.</p></section><div class="toolbar"><input id="exSearch" class="search" 'Cerca mostra, luogo, città, curatore…')}"></div><div id="exGrid" class="grid">${db.exhibitions.map(exhibitionCard).join('')||empty('🏛️','Non hai ancora inserito mostre.','<button class="btn primary" data-action="newExhibition">Crea la prima mostra</button>')}</div>`}
