...
function exhibitionModal(x={}){const arts=artworkPickerHtml(x.artworkIds||[],'exhibitionArts');openModal(x.id?'Modifica mostra':'Nuova mostra',`<div class="formgrid">${field('Titolo','title',x.title,'text','full')}${field('Data inizio','startDate',x.startDate,'date')}${field('Data fine','endDate',x.endDate,'date')}${field('Luogo / galleria','venue',x.venue)}${field('Città','city',x.city)}<div class="field"><label>Curatore / critico</label><input name="curator" list="proList" value="${esc(x.curator||'')}"><datalist id="proList">${(db.pros||[]).map(p=>`<option value="${esc(p.name||p.title||p.company||'')}">`).join('')}</datalist></div><div class="field"><label>Gallerista / galleria</label><input name="gallerist" list="galleryList" value="${esc(x.gallerist||'')}"><datalist id="galleryList">${(db.galleries||[]).map(g=>`<option value="${esc(g.name||g.title||g.company||'')}">`).join('')}</datalist></div>${field('Stato','status',x.status||'In programma')}${area('Descrizione','description',x.description)}${area('Note organizzative','notes',x.notes)}<div class="field full"><label>Locandina della mostra</label><input name="poster" type="file" accept="image/*">${x.poster?`<div style="margin-top:8px"><img src="${x.poster}" style="max-width:160px;border-radius:8px;border:1px solid var(--line)"><label class="chkline" style="margin-top:6px"><input type="checkbox" name="rimuoviPoster"> Rimuovi locandina</label></div>`:''}</div><div class="field full"><label>Opere partecipanti</label>${arts}</div></div>`,async fd=>{const pf=fd.get('poster');const nuovoPoster=pf&&pf.size?await fileData(pf):(fd.get('rimuoviPoster')?'':x.poster||'');const o={...x,id:x.id||uid(),title:fd.get('title'),startDate:fd.get('startDate'),endDate:fd.get('endDate'),venue:fd.get('venue'),city:fd.get('city'),curator:fd.get('curator'),gallerist:fd.get('gallerist'),status:fd.get('status'),description:fd.get('description'),notes:fd.get('notes'),poster:nuovoPoster,artworkIds:fd.getAll('arts'),updated:new Date().toISOString(),created:x.created||new Date().toISOString()};if(x.id)db.exhibitions=db.exhibitions.map(v=>v.id===x.id?o:v);else db.exhibitions.unshift(o);await save();modal.close();render();toast('Mostra salvata')});bindArtworkPicker('exhibitionArts')}
