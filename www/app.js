...
function exhibitionModal(x={}){
  ...
  const pros=(db.pros||[]).map(p=>({...p,name:(p.name||p.nome||p.fullName||p.title||'').trim(),role:p.role||p.ruolo||'',org:p.org||p.organization||p.galleria||''})).filter(p=>p&&p.name).sort((a,b)=>String(a.name).localeCompare(String(b.name),'it'));
