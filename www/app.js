...
function exhibitionModal(x={}){
  ...
  const pros=(db.pros||[]).filter(p=>p&&p.name).sort((a,b)=>String(a.name).localeCompare(String(b.name),'it'));
  const curatorValue=x.curatorId||'';
  const curatorOptions=pros.map(p=>`<option value="${p.id}" ${curatorValue===p.id?'selected':''}>${esc(p.name)}${p.role?' · '+esc(p.role):''}${p.org?' · '+esc(p.org):''}</option>`).join('');
  openModal(x.id?'Modifica mostra':'Nuova mostra',`<div class="formgrid">${field('Titolo','title',x.title,'text','full')}${field('Data inizio','startDate',x.startDate,'date')}${field('Data fine','endDate',x.endDate,'date')}${field('Luogo / galleria','venue',x.venue)}${field('Città','city',x.city)}
  <div class="field full"><label>Curatore / critico già registrato</label><div class="field-inline"><select name="curatorId" id="exCuratorSelect"><option value="">Nessuno / inserisci manualmente</option>${curatorOptions}<option value="__new__">＋ Aggiungi nuovo curatore / critico</option></select><button type="button" class="btn" id="exNewCuratorBtn">Nuovo</button></div><small class="meta">Sono mostrati i contatti salvati in “Curatori e critici”.</small></div>
  <div class="field full"><label>Nome curatore / critico</label><input name="curator" id="exCuratorName" value="${esc(x.curator||'')}" placeholder="Seleziona dall’elenco oppure scrivi un nuovo nome"></div>
  <div class="field"><label>Gallerista / galleria</label><input name="gallerist" list="galleryList" value="${esc(x.gallerist||'')}"><datalist id="galleryList">${(db.galleries||[]).map(g=>`<option value="${esc(g.name||g.title||g.company||'')}">`).join('')}</datalist></div>${field('Stato','status',x.status||'In programma')}${area('Descrizione','description',x.description)}${area('Note organizzative','notes',x.notes)}<div class="field full"><label>Locandina della mostra</label><input name="poster" type="file" accept="image/*">${x.poster?`<div style="margin-top:8px"><img src="${x.poster}" style="max-width:160px;border-radius:8px;border:1px solid var(--line)"><label class="chkline" style="margin-top:6px"><input type="checkbox" name="rimuoviPoster"> Rimuovi locandina</label></div>`:''}</div><div class="field full exhibition-artworks-box"><div class="row spread"><label>Opere partecipanti</label><span class="badge">Cerca e seleziona</span></div>${arts}</div></div>`,async fd=>{
    const pf=fd.get('poster');const nuovoPoster=pf&&pf.size?await fileData(pf):(fd.get('rimuoviPoster')?'':x.poster||'');
    let curatorId=fd.get('curatorId');if(curatorId==='__new__')curatorId='';
    const selectedPro=pros.find(p=>p.id===curatorId);const curator=(selectedPro?.name||fd.get('curator')||'').trim();
    const o={...x,id:x.id||uid(),title:fd.get('title'),startDate:fd.get('startDate'),endDate:fd.get('endDate'),venue:fd.get('venue'),city:fd.get('city'),curatorId,curator,gallerist:fd.get('gallerist'),status:fd.get('status'),description:fd.get('description'),notes:fd.get('notes'),poster:nuovoPoster,artworkIds:fd.getAll('arts'),updated:new Date().toISOString(),created:x.created||new Date().toISOString()};if(x.id)db.exhibitions=db.exhibitions.map(v=>v.id===x.id?o:v);else db.exhibitions.unshift(o);await save();modal.close();render();toast('Mostra salvata')
  });
  bindArtworkPicker('exhibitionArts');
  const sel=document.getElementById('exCuratorSelect'),name=document.getElementById('exCuratorName'),btn=document.getElementById('exNewCuratorBtn');
  const syncCurator=()=>{const p=pros.find(v=>v.id===sel?.value);if(p&&name)name.value=p.name;if(sel?.value==='__new__'&&name){name.value='';name.focus();}};
  sel?.addEventListener('change',syncCurator);btn?.addEventListener('click',()=>{if(sel)sel.value='__new__';syncCurator();});
}
...
async function backupWritePart(folder,fileName,text){
  ...
  if(FS){
    ...
    return {native:true,path:`Documenti/${folder}/${fileName}`};
  }
...
async function creaBackupRapido(){
  ...
  try{
    ...
    await backupWritePart(folder,'backup_rapido.mair',text);
    backupProgress(100,'Backup rapido completato in Documenti/'+folder);
    db.settings.lastBackup=new Date().toISOString();save();
    setTimeout(()=>{try{modal.close()}catch{};alert('Backup rapido completato.\n\nCartella: Documenti/'+folder+'\n\nNon contiene le immagini originali.')},400);
    return true;
  ...
  try{
    const writeRecords=async(section,records)=>{
      ...
      const flush=async()=>{
        ...
        await backupWritePart(folder,name,text);
        files.push({name,section,records:chunk.length,checksum:checksumString(text),chars:text.length});
        chunk=[];chars=0;
      };
    ...
    const manifest={format:'MAIR_GO_BACKUP_MULTIPART',version:16,createdAt:new Date().toISOString(),app:'MAIR GO!',targetArtworks:1000,partMaxChars:BACKUP_PART_MAX_CHARS,counts:contaSezioni(db),files};
    const manifestText=JSON.stringify(manifest,null,2);
    await backupWritePart(folder,'manifest.mairindex',manifestText);
    backupProgress(100,`Completato: ${files.length} parti + manifest`);
    db.settings.lastBackup=new Date().toISOString();save();
    setTimeout(()=>{try{modal.close()}catch{};alert(`Backup completo creato.\n\nCartella: Documenti/${folder}\nFile: ${files.length} parti + manifest.mairindex\n\nCopia l’intera cartella su Drive o PC.`)},500);
    return true;
...
function backupReadFilesInput(){return new Promise((resolve,reject)=>{const i=document.createElement('input');i.type='file';i.multiple=true;i.accept='.mairindex,.mairpart,application/json,*/*';i.onchange=()=>i.files&&i.files.length?resolve([...i.files]):reject(new Error('Nessun file selezionato'));i.click()})}
async function ripristinaBackupMultiparte(){
  try{
    const files=await backupReadFilesInput();
    const manifestFile=files.find(f=>f.name.endsWith('.mairindex'));
    if(!manifestFile)throw new Error('Seleziona anche manifest.mairindex.');
    const manifest=JSON.parse(await manifestFile.text());
    if(manifest.format!=='MAIR_GO_BACKUP_MULTIPART')throw new Error('Manifest non valido.');
    const byName=new Map(files.map(f=>[f.name,f]));
    const missing=(manifest.files||[]).filter(x=>!byName.has(x.name));
    if(missing.length)throw new Error(`Mancano ${missing.length} parti. Seleziona tutti i file della cartella.`);
    ...
    for(let n=0;n<manifest.files.length;n++){
      const info=manifest.files[n],text=await byName.get(info.name).text();
      if(checksumString(text)!==info.checksum)throw new Error('Parte danneggiata: '+info.name);
