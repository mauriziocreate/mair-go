...
function bindArtworkPicker(prefix='artPick'){
  ...
  const update=()=>{const q=(input.value||'').trim().toLowerCase();let shown=0;cards.forEach(c=>{const ok=!q||c.dataset.search.includes(q);c.hidden=!ok;if(ok)shown++;});count.textContent=`${shown} opere visualizzate · ${cards.filter(c=>c.querySelector('input').checked).length} selezionate`;};
...
function exhibitionModal(x={}){
  ...
  const curatori=(db.pros||[]).filter(p=>['Curatore','Critico d’arte','Critico d\'arte','Organizzatore','Gallerista','Altro'].includes(p.role)||!p.role);
  const curatoreId=x.curatorId||((db.pros||[]).find(p=>(p.name||'').trim()===(x.curator||'').trim())?.id||'');
  const curatoreOptions=curatori.map(p=>`<option value="${p.id}" ${curatoreId===p.id?'selected':''}>${esc(p.name||'Senza nome')}${p.role?' · '+esc(p.role):''}${p.org?' · '+esc(p.org):''}</option>`).join('');
  openModal(x.id?'Modifica mostra':'Nuova mostra',`<div class="formgrid">
    ${field('Titolo','title',x.title,'text','full')}
    ${field('Data inizio','startDate',x.startDate,'date')}${field('Data fine','endDate',x.endDate,'date')}
    ${field('Luogo / galleria','venue',x.venue)}${field('Città','city',x.city)}
    <div class="field full"><label>Curatore / critico</label>
      <select name="curatorId" id="exCuratorSelect"><option value="">Nessuno</option>${curatoreOptions}<option value="__new__">＋ Aggiungi nuovo curatore / critico</option></select>
      <p class="meta">Scegli un contatto già presente in “Curatori e critici”, oppure aggiungilo qui.</p>
    </div>
    <div id="exNewCurator" class="field full" hidden><div class="formgrid curator-inline">
      ${field('Nome e cognome','newCuratorName','','text','full')}
      <div class="field"><label>Ruolo</label><select name="newCuratorRole"><option>Curatore</option><option>Critico d’arte</option><option>Organizzatore</option><option>Gallerista</option><option>Altro</option></select></div>
      ${field('Email','newCuratorEmail','','email')}${field('Telefono','newCuratorPhone')}${field('Istituzione / organizzazione','newCuratorOrg')}
    </div></div>
    <div class="field"><label>Gallerista / galleria</label><input name="gallerist" list="galleryList" value="${esc(x.gallerist||'')}"><datalist id="galleryList">${(db.galleries||[]).map(g=>`<option value="${esc(g.name||g.title||g.company||'')}">`).join('')}</datalist></div>
    ${field('Stato','status',x.status||'In programma')}${area('Descrizione','description',x.description)}${area('Note organizzative','notes',x.notes)}
    ...
    <div class="field full"><label>Opere partecipanti</label><p class="meta">Usa la ricerca per titolo, codice, anno o tecnica; le opere selezionate restano visibili anche mentre filtri.</p>${arts}</div>
  </div>`,async fd=>{
    const pf=fd.get('poster');
    const nuovoPoster=pf&&pf.size?await fileData(pf):(fd.get('rimuoviPoster')?'':x.poster||'');
    let selectedCuratorId=fd.get('curatorId')||'';
    let curator='';
    if(selectedCuratorId==='__new__'){
      const name=String(fd.get('newCuratorName')||'').trim();
      if(!name)throw new Error('Inserisci il nome del nuovo curatore o critico.');
      const email=String(fd.get('newCuratorEmail')||'').trim();
      if(!emailValida(email))throw new Error('L’email del curatore non è valida.');
      const nuovo={id:uid(),name,role:fd.get('newCuratorRole')||'Curatore',email,phone:fd.get('newCuratorPhone'),org:fd.get('newCuratorOrg'),created:new Date().toISOString(),updated:new Date().toISOString()};
      db.pros.unshift(nuovo); selectedCuratorId=nuovo.id; curator=nuovo.name;
    }else if(selectedCuratorId){
      curator=(db.pros||[]).find(p=>p.id===selectedCuratorId)?.name||'';
    }
    const o={...x,id:x.id||uid(),title:fd.get('title'),startDate:fd.get('startDate'),endDate:fd.get('endDate'),venue:fd.get('venue'),city:fd.get('city'),curatorId:selectedCuratorId,curator,gallerist:fd.get('gallerist'),status:fd.get('status'),description:fd.get('description'),notes:fd.get('notes'),poster:nuovoPoster,artworkIds:fd.getAll('arts'),updated:new Date().toISOString(),created:x.created||new Date().toISOString()};
    if(x.id)db.exhibitions=db.exhibitions.map(v=>v.id===x.id?o:v);else db.exhibitions.unshift(o);
    await save();modal.close();render();toast('Mostra salvata');
  });
  bindArtworkPicker('exhibitionArts');
  const sel=$('#exCuratorSelect'),box=$('#exNewCurator');
  const toggle=()=>{box.hidden=sel.value!=='__new__';};
...
function emailValida(v){
  v=String(v||'').trim();
  return !v || /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i.test(v);
}
function validaEmailCampo(input){
  if(!input)return true;
  const ok=emailValida(input.value);
  input.setCustomValidity(ok?'':'Inserisci un indirizzo email valido, per esempio nome@dominio.it');
  return ok;
}
function validaEmailForm(form){
  let ok=true;
  form.querySelectorAll('input[type="email"]').forEach(i=>{if(!validaEmailCampo(i))ok=false;});
...
function openModal(title,html,onSave,saveLabel='Salva'){
  ...
  saveBtn.onclick=async e=>{
    ...
    if(!validaEmailForm(form) || !form.reportValidity())return;
...
function bulkImportArtworksModal(){
  ...
  openModal('Importa più opere',`
    ...
    <div class="formgrid">
      <div class="field full"><label>Fotografie delle opere</label><input id="bulkArtworkFiles" name="bulkArtworkFiles" type="file" accept="image/*" multiple required><small>Massimo 5 MB per immagine. Gli originali restano integri; viene creata una miniatura ad alta qualità.</small></div>
      ...
      const chosen=valid.map((f,i)=>({file:f,title:rows[i]?.querySelector('[data-bulk-title]')?.value.trim()||titoloDaNomeFile(f.name),included:rows[i]?.querySelector('[data-bulk-include]')?.checked!==false})).filter(x=>x.included&&x.file.size<=IMG_MAX_BYTES);
      if(!chosen.length){alert('Non ci sono immagini importabili selezionate. Deseleziona quelle oltre 5 MB oppure scegli altri file.');return;}
  ...
  if(inp)inp.onchange=()=>{
    ...
    box.innerHTML=files.map((f,i)=>`<div class="bulk-artwork-row ${f.size>IMG_MAX_BYTES?'invalid':''}"><label class="bulk-include"><input type="checkbox" data-bulk-include ${f.size>IMG_MAX_BYTES?'':'checked'} ${f.size>IMG_MAX_BYTES?'disabled':''}><span>${f.size>IMG_MAX_BYTES?'Esclusa':'Includi'}</span></label><div class="bulk-file-info"><strong>${esc(f.name)}</strong><small>${(f.size/1048576).toFixed(2)} MB${f.size>IMG_MAX_BYTES?' · supera 5 MB: esclusa automaticamente':''}</small></div><input data-bulk-title value="${esc(titoloDaNomeFile(f.name))}" aria-label="Titolo opera ${i+1}" ${f.size>IMG_MAX_BYTES?'disabled':''}></div>`).join('');
  };
}
