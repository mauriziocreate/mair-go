...
const I18N={
  ...
  "Catalogo con molte opere":"Catalog with many artworks",
  "Genero":"Generating",
  "Salvata parte":"Saved part",
};
...
function clientsView(){return `${section('Clienti','<button class="btn primary" data-action="newClient">＋ Nuovo cliente</button>')}<div class="toolbar"><input id="clientSearch" class="search" 'Cerca nome, email, città, preferenze…')}"></div><div id="clientGrid" class="grid">${db.clients.map(clientCard).join('')||empty('👥','Nessun cliente registrato.','<button class="btn primary" data-action="newClient">Aggiungi il primo cliente</button>')}</div>`}
...
function salesView(){const total=db.sales.reduce((n,s)=>n+Number(s.total||0),0),paid=db.sales.reduce((n,s)=>n+Number(s.paid||0),0);return `${section('Vendite','<button class="btn primary" data-action="newSale">＋ Registra vendita</button>')}<div class="stats">${stat('Operazioni',db.sales.length,'🧾')}${stat('Totale',euro(total),'💶')}${stat('Incassato',euro(paid),'✓')}${stat('Da incassare',euro(Math.max(0,total-paid)),'◷')}</div><div class="toolbar"><input id="saleSearch" class="search" 'Cerca opera, cliente, pagamento…')}"></div><div id="saleGrid" class="grid">${db.sales.map(saleCard).join('')||empty('💶','Nessuna vendita registrata.','<button class="btn primary" data-action="newSale">Registra la prima vendita</button>')}</div>`}
...
function linksView(){
  ...
  return section('Link utili','<button class="btn primary" data-action="newLink">&#65291; Nuovo link</button>')
   ...
   +'<div class="toolbar"><input id="linkSearch" class="search" placeholder="${T("Cerca per titolo, indirizzo, nota, categoria\\u2026")}"></div>'
...
function proView(){
  ...
  return section('Curatori e critici','<button class="btn primary" data-action="newPro">&#65291; Nuovo contatto</button>')
   ...
   +'<div class="toolbar"><input id="proSearch" class="search" placeholder="${T("Cerca per nome, ruolo, galleria, citt\\u00e0, telefono, email\\u2026")}"><select id="proRole"><option value="">Tutti i ruoli</option>'+PRO_RUOLI.map(r=>'<option>'+r+'</option>').join('')+'</select></div>'
...
function certificatesView(){
  ...
  <div class="toolbar"><input id="certSearch" class="search" placeholder="${T("Cerca per titolo, numero, opera, acquirente, luogo…")}"></div>
...
// menu di scelta quando il catalogo ha molte opere
function scegliModalitaCatalogo(nOp){
  return new Promise(risolvi=>{
    ...
    // il pulsante Salva del modale non serve qui
    ...
    document.querySelectorAll('#modalBody [data-scelta]').forEach(b=>b.onclick=()=>{
      ...
      const s=b.dataset.scelta;modal.close();
      if(s==='unico-normale')risolvi({modo:'unico',q:QN});
      else if(s==='unico-ridotta')risolvi({modo:'unico',q:QR});
      else if(s==='diviso')risolvi({modo:'diviso',q:QR});
    });
    // se chiude senza scegliere
...
// genera il catalogo diviso in più PDF da 100 opere, salvati singolarmente
async function generaCatalogoDiviso(dato,scelta,nOp){
  ...
  for(let i=0;i<parti;i++){
    ...
    try{
      ...
      await new Promise(r=>setTimeout(r,300)); // respiro per liberare memoria
    }catch(e){
...
async function pdfCatalogoImpaginato(p,opt){
  opt=opt||{};
  const qLato=opt.lato||1100;      // risoluzione immagini
  const qJpeg=opt.jpeg||0.82;       // compressione
  const C=pdfCfg();
  ...
  try{
    ...
    for(let i=0;i<arts.length;i++){
      ...
      if(a.image){
        try{
          const rid=await pdfImgRidotta(a.image,qLato,qJpeg);  // qualità scelta dall'utente
          if(rid&&rid.w&&rid.h){
      ...
      try{
        ...
        try{
          if(ctx.tipo==='catalogo'&&ctx.dato){
            ...
            // sopra le 120 opere: chiedo all'utente come procedere (mai crash)
            if(nOp>120){
              ...
              if(scelta.modo==='diviso'){
                ...
                return; // i file sono già salvati singolarmente
              }
