...
const HOME_HIDDEN_TILES=new Set(['timeline','settings','guide','info','contact']);
function homeView(){
  const h=homeCfg();
  ...
  const tiles=(h.tiles||[]).filter(k=>HOME_TILES[k]&&!HOME_HIDDEN_TILES.has(k)).map((k,idx)=>{const TI=HOME_TILES[k];const n=TI.count?TI.count():null;const nav=TI.action?`data-action="${TI.action}"`:`data-go="${k}"`;
...
function homeCustomizeModal(){
  ...
  const ordine=(h.tiles||[]).filter(k=>HOME_TILES[k]&&!HOME_HIDDEN_TILES.has(k));
  openModal('Personalizza la schermata principale',`<div class="formgrid">
    ...
    <div class="field full"><label>Sezioni da mostrare</label><div class="chkgrid">${chk('tiles',Object.fromEntries(Object.entries(HOME_TILES).filter(([k])=>!HOME_HIDDEN_TILES.has(k))),h.tiles||[])}</div></div>
    ...
    let tiles=fd.getAll('tiles').filter(k=>!HOME_HIDDEN_TILES.has(k));
...
function moreView(){
  ...
  <div class="hub-section"><h3>App e sicurezza</h3><div class="hub-grid compact">
    ...
    <button class="hub-tile" data-action="dona"><span class="hub-icon">❤️</span><span class="hub-copy"><strong>Sostieni MAIR GO!</strong><small>Contribuisci allo sviluppo dell’app gratuita e senza pubblicità.</small></span><span class="hub-arrow">›</span></button>
  </div></div>`;
}
